...
extern unsigned int funcLoadSize;
extern unsigned int * funcLoadEnd;
extern unsigned int * funcLoadStart;
extern unsigned int * funcRunStart;
#pragma CODE_SECTION(outputEnable, ".text2")
...
int main(void) {
	//memcpy(&funcRunStart, &funcLoadStart, funcLoadSize);
	memcpy2(&funcRunStart, &funcLoadStart, &funcLoadEnd);
	DisableDog();
	InitSysCtrl();
	EALLOW;
	EALLOW;
...
void memcpy2(unsigned int * dest, unsigned int* source, unsigned int* sourceEnd){
